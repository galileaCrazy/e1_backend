...
const modules = {
  citas: {
    ...
    clientFiltered: true,
    list: ({ orgId, role }) => role === "PACIENTE" ? "/api/citas/mis-citas" : `/api/citas/organizacion/${orgId}`,
    endpoint: "/api/citas",
    columns: [
    ...
    filters: [
      ...
      { key: "medicoId", label: "Medico", type: "combobox", source: "medicos", live: true },
...
function ModuleView({ moduleKey, config, context }) {
  ...
  }, [rows, filters, liveFilters, support, config.clientFiltered]);
...
function FilterBar({ config, filters, support, loading, onApply, onClear, onLiveChange, resetKey }) {
  ...
  return (
    <section className="section filter-section">
      <form
        ...
        className="filters"
        onChange={(e) => {
          const fieldCfg = (config.filters || []).find((f) => f.key === e.target.name && f.live && f.type !== "combobox");
          if (fieldCfg && onLiveChange) onLiveChange(e.target.name, e.target.value);
        }}
        onSubmit={(event) => {
          event.preventDefault();
          const applyFields = (config.filters || []).filter((f) => f.type !== "combobox");
...
function DataTable({ rows, columns, support = {}, onEdit, onDelete, onChangeStatus, onToggleActive, togglingId, emptyLabel, emptyHint }) {
      ...
      <table>
        <thead>
          <tr>
            ...
            {(onEdit || onDelete || onChangeStatus || onToggleActive) && <th className="right">Acciones</th>}
          </tr>
        </thead>
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className={onToggleActive && row.activo === false ? "row-inactive" : ""}>
              ...
              {(onEdit || onDelete || onChangeStatus || onToggleActive) && (
                <td>
